...
const { isValid, isValidRequestBody, isvalidEmail, moblieRegex } = require('../validatores/validation');
const validator = require('../validatores/validation');
...
const createBooks = async function(req, res) {
    try {
        ...
        const { title, excerpt, userId, ISBN, category, subcategory, deletedAt, releasedAt } = requestbody;
        ...
        if (uniqueTitle) {
            return res.status(400).send({ status: false, message: "Title is already present plz provide unique title" })
        }
        ...
        if (!user) {
            return res.status(400).send({ status: true, message: "user does not exist" })
        }
        ...
        if (!validator.isValid(category)) {
            return res.status(400).send({ status: false, message: " plz provide category " })
        }
        ...
        if (!validator.isValid(releasedAt)) {
            return res.status(400).send({ status: false, message: "relesedAt must be present " })
        }
        const newbookdata = await bookModel.create(requestbody)
        return res.status(201).send({ status: true, message: 'successfully created bookdata', data: newbookdata })
    } catch {
